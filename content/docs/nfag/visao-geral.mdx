...
A chave de acesso da NFAg possui **44 caracteres**. O schema (`Id` no padrão `NFAG[0-9]{6}[A-Z0-9]{12}[0-9]{26}`) define o segmento de 12 posições do **CNPJ do emitente** como **alfanumérico** (`[A-Z0-9]{12}`): a NFAg já adota CNPJ alfanumérico, portanto a chave não é totalmente numérica.
...
No schema, o único evento de contribuinte com leiaute próprio é o **cancelamento** (`evCancNFAg`, `descEvento` fixo `Cancelamento`); ele compartilha o namespace `http://www.portalfiscal.inf.br/nfag` e o atributo `versao` fixo `1.00`. Os eventos de marcação `240xxx` são lavrados pelo fisco.
...
| Campo | Valor |
|---|---|
| Documento | MOC NFAg — Padrões Técnicos de Comunicação, versão 1.00k — 11 de março de 2026, p. 6–58. |
| Versão | 1.00k |
| Data | março de 2026 |
...
Schema: nfag_v1.00 · eventoNFAg_v1.00 · evCancNFAg_v1.00 (PL_NFAg_1.00j)
