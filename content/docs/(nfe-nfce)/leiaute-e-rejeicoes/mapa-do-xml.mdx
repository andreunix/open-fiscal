...
| UB 🔄 | `IS` / `IBSCBS` | Imposto Seletivo e tributação IBS/CBS do item |
| VB 🔄 | `vItem` | total do item já composto pelos tributos da RTC |
| VC 🔄 | `DFeReferenciado` | chave e item do documento fiscal original |
...
Os limites de ocorrência abaixo vêm direto do schema (`minOccurs`/`maxOccurs`). Um gerador deve recusar listas maiores que o teto antes mesmo de enviar — a SEFAZ rejeita por schema.

| Grupo | Tag | Ocorrência (XSD) |
|---|---|---|
| BA | `NFref` | 0–999 |
| GA | `autXML` | 0–10 |
| H | `det` | 1–990 |
| I01 | `DI` | 0–100 |
| I02 | `adi` | 1–999 (por `DI`) |
| I80 | `rastro` | 0–500 |
| L | `arma` | 1–500 |
| X | `reboque` | 0–5 |
| X | `vol` | 0–5000 |
| X | `lacres` | 0–5000 (por `vol`) |
| YA | `detPag` | 1–100 |
| Z | `obsCont` / `obsFisco` | 0–10 cada |
| Z | `procRef` | 0–100 |

Grupos sem `maxOccurs` (ex.: `infIntermed`, `infAdic`, `infRespTec`, `infNFeSupl`) são `0–1`; `pag` é único e contém os `detPag`.
...
| Campo | Valor |
|---|---|
| Documento | MOC 7.0 — Anexo I, capítulo 2 (Leiaute da NF-e), p. 8–66. |
| Versão | v1.50 |
| Data | 03/06/2026 |
| Páginas/capítulo | p. 8–66; capítulo 2 (Leiaute da NF-e), p |
...
MOC 7.0 — Anexo I, capítulo 2 (Leiaute da NF-e), p. 8–66. Overlay RTC: NT 2025.002 v1.50 (03/06/2026).
